...
class UserController extends AbstractActionController
{
     ...
     * @var string
     */
    protected $failedLoginMessage = 'Authentication failed. Please try again111.';
    ...
    public function loginAction()
    {
        ...
        if (!$form->isValid()) {
			...
			$logger = new Logger();
			$logger->addWriter($writer);		
			$logger->info('Form validate success');
		}
    ...
    public function authenticateAction()
    {
        ...
        if (!$auth->isValid()) {
			$logger->info('invalida user/pass ');
            $this->flashMessenger()->setNamespace('zfcuser-login-form')->addMessage($this->failedLoginMessage);
    ...
    public function registerAction()
    {
        ...
        if ($service->getOptions()->getLoginAfterRegistration()) {
            ...
            if (in_array('parent_email', $identityFields)) {
                $post['identity'] = $user->getParentEmail();
    ...
    public function changeEmailAction()
    {
        ...
        $request = $this->getRequest();
        $request->getPost()->set('identity', $this->getUserService()->getAuthService()->getIdentity()->getParentEmail());
